...
namespace cppcrypto
{
	...
	void dump_state(const char* name, uint64_t* y, int elements = 16)
	{
		printf("%s: \n", name);
...
#ifdef CPPCRYPTO_DEBUG
	dump_state("init", h, 32);
#endif
};

void echo::update(const unsigned char* data, size_t len)
{
	size_t limit = bs / 8;
	while (pos + len >= limit)
	{
		memcpy(m + pos, data, limit - pos);
		transform(true, (limit - pos) * 8);
		len -= limit - pos;
		total += (limit - pos) * 8;
		data += limit - pos;
		pos = 0;
...
void echo::final(unsigned char* hash)
{
#ifdef CPPCRYPTO_DEBUG
	dump_state("pre-final", h, 32);
#endif
    ...
    memcpy(hash, reinterpret_cast<unsigned char*>(h.get()), hashsize() / 8);

#ifdef CPPCRYPTO_DEBUG
	dump_state("post-final", h, 32);
#endif
}

void echo::transform(bool addedbits, uint64_t addtototal)
{
	if (impl_)
		return impl_->transform(h.get(), salt.get(), total, addedbits, addtototal);

    uint64_t w[32];

    uint64_t counter = 0;
    if (addedbits)
        counter = total + addtototal;
    memcpy(w, h.get(), sizeof(w));
    ...
    for (int l = 0; l < rounds; l++)
    {
        ...
        {
#ifdef CPPCRYPTO_DEBUG
            dump_state("AES ROUND BEG", w, 32);
#endif

            size_t idx = r * 2;
            size_t idx1 = r * 2 + 1;
            uint32_t t0 = static_cast<uint32_t>(counter)
                ^ T[0][(unsigned char)(w[idx])]
                ^ T[1][(unsigned char)(w[idx] >> 40)]
                ^ T[2][(unsigned char)(w[idx1] >> 16)]
                ^ T[3][(unsigned char)(w[idx1] >> 56)];
            uint32_t t1 = (counter >> 32)
                ^ T[0][(unsigned char)(w[idx] >> 32)]
                ^ T[1][(unsigned char)(w[idx1] >> 8)]
                ^ T[2][(unsigned char)(w[idx1] >> 48)]
                ^ T[3][(unsigned char)(w[idx] >> 24)];
            uint32_t t2 = T[0][(unsigned char)(w[idx1])]
                ^ T[1][(unsigned char)(w[idx1] >> 40)]
                ^ T[2][(unsigned char)(w[idx] >> 16)]
                ^ T[3][(unsigned char)(w[idx] >> 56)];
            uint32_t t3 = T[0][(unsigned char)(w[idx1] >> 32)]
                ^ T[1][(unsigned char)(w[idx] >> 8)]
                ^ T[2][(unsigned char)(w[idx] >> 48)]
                ^ T[3][(unsigned char)(w[idx1] >> 24)];

            ++counter;

            w[idx] = (uint64_t)(T[0][(unsigned char)(t0)]
                ^ T[1][(unsigned char)(t1 >> 8)]
                ^ T[2][(unsigned char)(t2 >> 16)]
                ^ T[3][(unsigned char)(t3 >> 24)])
                ^ (
                    (uint64_t)(T[0][(unsigned char)(t1)]
                        ^ T[1][(unsigned char)(t2 >> 8)]
                        ^ T[2][(unsigned char)(t3 >> 16)]
                        ^ T[3][(unsigned char)(t0 >> 24)])
                    << 32)
                ^ salt[0];
            w[idx + 1] = (uint64_t)(T[0][(unsigned char)(t2)]
                ^ T[1][(unsigned char)(t3 >> 8)]
                ^ T[2][(unsigned char)(t0 >> 16)]
                ^ T[3][(unsigned char)(t1 >> 24)])
                ^ (
                    (uint64_t)(T[0][(unsigned char)(t3)]
                        ^ T[1][(unsigned char)(t0 >> 8)]
                        ^ T[2][(unsigned char)(t1 >> 16)]
                        ^ T[3][(unsigned char)(t2 >> 24)])
                    << 32)
                ^ salt[1];
        }
#ifdef CPPCRYPTO_DEBUG
        dump_state("SUBWORDS END", w, 32);
#endif
...
#ifdef CPPCRYPTO_DEBUG
        dump_state("MIXCOLUMNS END", w, 32);
#endif
    }
...
echo::echo(size_t hashsize, const unsigned char* salt, size_t saltlen)
	: hs(hashsize)
    , bs(hashsize > 256 ? 1024 : 1536)
    , m(reinterpret_cast<unsigned char*>(h.get()) + (2048 - bs) / 8)
{
	...
	{
		if (hashsize > 256)
			impl_.create<detail::echo_impl_aesni_512>();
		else
			impl_.create<detail::echo_impl_aesni_256>();
	}
#endif
}
...
//	zero_memory(salt.get(), salt.bytes());
}

void echo::validate_salt_length(size_t saltlen) const
{
	if (saltlen && saltlen != 16)
		throw std::runtime_error("invalid salt length");
}

echo* echo::clone() const 
{ 
    return new echo(hs, reinterpret_cast<const unsigned char*>(salt.get()), 16); 
}
