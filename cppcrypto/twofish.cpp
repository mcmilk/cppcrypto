...
namespace cppcrypto
{
	...
	static const uint32_t RS[8][256] = {
		{
	...
	bool twofish128::init(const uint8_t* key, block_cipher::direction direction)
	{
		...
		for (int i = 0; i < 256; ++i)
		{
			s[0][i] = MDSQ[0][Q[0][Q[0][i] ^ us.S8[0]] ^ us.S8[4]];
			s[1][i] = MDSQ[1][Q[0][Q[1][i] ^ us.S8[1]] ^ us.S8[5]];
			s[2][i] = MDSQ[2][Q[1][Q[0][i] ^ us.S8[2]] ^ us.S8[6]];
			s[3][i] = MDSQ[3][Q[1][Q[1][i] ^ us.S8[3]] ^ us.S8[7]];
		}
...
#define ROUNDT(x0, x1, r) \
	f0 = s[0][x0 & 0xFF] ^ s[1][(x0 >> 8) & 0xFF] ^ s[2][(x0 >> 16) & 0xFF] ^ s[3][(x0 >> 24) & 0xFF]; \
	f1 = s[0][(x1 >> 24) & 0xFF] ^ s[1][x1 & 0xFF] ^ s[2][(x1 >> 8) & 0xFF] ^ s[3][(x1 >> 16) & 0xFF]; \
	f0 += f1; \
	f1 += f0 + rk[2 * r + 9]; \
	f0 += rk[2 * r + 8];
...
#define RROUNDB(r) \
	...
	namespace detail
	{
		...
		void twofish::encrypt_block(const uint8_t* in, uint8_t* out)
		{
			...
			uint32_t x2 = *(((const uint32_t*)in) + 2) ^ rk[2];
			uint32_t x3 = *(((const uint32_t*)in) + 3) ^ rk[3];
			uint32_t f0, f1;
		...
		void twofish::decrypt_block(const uint8_t* in, uint8_t* out)
		{
			...
			uint32_t x2 = *(((const uint32_t*)in) + 2) ^ rk[6];
			uint32_t x3 = *(((const uint32_t*)in) + 3) ^ rk[7];
			uint32_t f0, f1;
	...
	bool twofish192::init(const uint8_t* key, block_cipher::direction direction)
	{
		...
		for (int i = 0; i < 256; ++i)
		{
			s[0][i] = MDSQ[0][Q[0][Q[0][Q[1][i] ^ us.S8[0]] ^ us.S8[4]] ^ us.S8[8]];
			s[1][i] = MDSQ[1][Q[0][Q[1][Q[1][i] ^ us.S8[1]] ^ us.S8[5]] ^ us.S8[9]];
			s[2][i] = MDSQ[2][Q[1][Q[0][Q[0][i] ^ us.S8[2]] ^ us.S8[6]] ^ us.S8[10]];
			s[3][i] = MDSQ[3][Q[1][Q[1][Q[0][i] ^ us.S8[3]] ^ us.S8[7]] ^ us.S8[11]];
		}
	...
	bool twofish256::init(const uint8_t* key, block_cipher::direction direction)
	{
		...
		for (int i = 0; i < 256; ++i)
		{
			s[0][i] = MDSQ[0][Q[0][Q[0][Q[1][Q[1][i] ^ us.S8[0]] ^ us.S8[4]] ^ us.S8[8]] ^ us.S8[12]];
			s[1][i] = MDSQ[1][Q[0][Q[1][Q[1][Q[0][i] ^ us.S8[1]] ^ us.S8[5]] ^ us.S8[9]] ^ us.S8[13]];
			s[2][i] = MDSQ[2][Q[1][Q[0][Q[0][Q[0][i] ^ us.S8[2]] ^ us.S8[6]] ^ us.S8[10]] ^ us.S8[14]];
			s[3][i] = MDSQ[3][Q[1][Q[1][Q[0][Q[1][i] ^ us.S8[3]] ^ us.S8[7]] ^ us.S8[11]] ^ us.S8[15]];
		}
	...
	void generate_tables()
	{
		...
		for (int q = 0; q < 2; q++)
		{
			...
			for (int x = 0; x < 256; x++)
			{
				...
				if (!(x % 16))
					std::cout << "\n\t\t";
				std::cout << "0x" << std::setfill('0') << std::setw(2) << std::hex << y;
				if (x != 255)
					std::cout << ",";
			}
			std::cout << "\n\t}";
		...
		// generate MDS tables
		std::cout << "static const uint32_t MDSQ[4][256] = {\n";
		for (int column = 0; column < 4; column++)
		{
			...
			for (int y = 0; y < 256; y++)
			{
				...
				if (column == 0 || column == 2)
					...
					yq = Q[0][y];
				val ^= mul_gf(mds[0][column], yq, 0x0169);
				val ^= mul_gf(mds[1][column], yq, 0x0169) << 8;
				val ^= mul_gf(mds[2][column], yq, 0x0169) << 16;
				val ^= mul_gf(mds[3][column], yq, 0x0169) << 24;
				if (!(y % 8))
					std::cout << "\n\t\t";
				std::cout << "0x" << std::setfill('0') << std::setw(8) << std::hex << val << 'U';
				if (y != 255)
					std::cout << ",";
			}
			std::cout << "\n\t}";
		...
		// generate RS tables
		std::cout << "static const uint32_t RS[8][256] = {\n";
		for (int column = 0; column < 8; column++)
		{
			...
			{
				uint32_t val = 0;
				val ^= mul_gf(rs[0][column], y, 0x014d);
				val ^= mul_gf(rs[1][column], y, 0x014d) << 8;
				val ^= mul_gf(rs[2][column], y, 0x014d) << 16;
				val ^= mul_gf(rs[3][column], y, 0x014d) << 24;
				if (!(y % 8))
					std::cout << "\n\t\t";
				std::cout << "0x" << std::setfill('0') << std::setw(8) << std::hex << val << 'U';
				if (y != 255)
