...
#define KEYGEN224STEPD(idx, rc) \
		...
		bool rijndael160_160_impl_aesni::init(const uint8_t* key, block_cipher::direction direction)
		{
			...
			if (direction == block_cipher::decryption)
			{
				...
				__m128i t2 = _mm_shuffle_epi32(_mm_blend_epi32(_mm_blend_epi32(_mm_slli_si128(rk[2], 4), rk[0], 1), rk[1], 8), _MM_SHUFFLE(0, 2, 1, 3)); // rk[13]
				...
				rk[14] = t1;
				t1 = _mm_shuffle_epi32(_mm_blend_epi32(_mm_blend_epi32(rk[2], _mm_srli_si128(rk[4], 8), 2), rk[3], 1), _MM_SHUFFLE(0, 3, 2, 1)); // rk[11]
				...
				rk[11] = t1;
				rk[10] = t2;
				t1 = _mm_shuffle_epi32(_mm_blend_epi32(_mm_blend_epi32(_mm_slli_si128(rk[7], 4), rk[6], 8), rk[5], 1), _MM_SHUFFLE(0, 2, 1, 3)); // rk[8]
		...
		bool rijndael160_224_impl_aesni::init(const uint8_t* key, block_cipher::direction direction)
		{
			...
			if (direction == block_cipher::decryption)
			{
				...
				rk[2] = _mm_shuffle_epi32(_mm_blend_epi32(_mm_blend_epi32(_mm_blend_epi32(_mm_srli_si128(rk[13], 4), rk[15], 8), _mm_slli_si128(rk[14], 4), 2), rk[16], 1), _MM_SHUFFLE(1, 2, 0, 3));
				
				rk[16] = t2;
				t2 = _mm_shuffle_epi32(_mm_blend_epi32(_mm_blend_epi32(_mm_blend_epi32(_mm_srli_si128(rk[3], 4), rk[5], 8), _mm_slli_si128(rk[4], 4), 2), rk[6], 1), _MM_SHUFFLE(1, 2, 0, 3)); // rk[12]
				...
				rk[7] = _mm_shuffle_epi32(_mm_blend_epi32(_mm_blend_epi32(_mm_blend_epi32(_mm_srli_si128(rk[10], 4), rk[8], 8), rk[9], 1), _mm_slli_si128(rk[11], 4), 2), _MM_SHUFFLE(0, 3, 1, 2));
				rk[8] = t4;
				rk[9] = t2;
				rk[10] = t1;
				rk[11] = t3;
		...
		static inline void rijndael224_swapimc(__m128i* rk)
		{
			...
			__m128i t3 = _mm_shuffle_epi32(_mm_blend_epi32(_mm_blend_epi32(_mm_slli_si128(rk[0], 8), rk[2], 8), rk[3], 3), _MM_SHUFFLE(2, 1, 0, 3)); // rk[22]
			...
			rk[19] = t2;
			t2 = _mm_shuffle_epi32(_mm_blend_epi32(_mm_blend_epi32(_mm_slli_si128(rk[7], 8), rk[9], 8), rk[10], 3), _MM_SHUFFLE(2, 1, 0, 3)); // rk[15]
			...
			rk[16] = t1;
			t1 = _mm_shuffle_epi32(_mm_blend_epi32(_mm_blend_epi32(_mm_srli_si128(rk[10], 4), rk[11], 1), rk[13], 8), _MM_SHUFFLE(0, 2, 1, 3)); // rk[12]
		...
		bool rijndael160_192_impl_aesni::init(const uint8_t* key, block_cipher::direction direction)
		{
			...
			if (direction == block_cipher::decryption)
			{
				...
				rk[14] = t2;
				t2 = _mm_shuffle_epi32(_mm_blend_epi32(_mm_blend_epi32(_mm_slli_si128(rk[4], 4), rk[6], 1), rk[3], 8), _MM_SHUFFLE(2, 1, 3, 0)); // rk[11]
		...
		bool rijndael160_256_impl_aesni::init(const uint8_t* key, block_cipher::direction direction)
		{
			...
			if (direction == block_cipher::decryption)
			{
				...
				rk[17] = t2;
				t2 = _mm_shuffle_epi32(_mm_blend_epi32(_mm_blend_epi32(rk[5], _mm_srli_si128(rk[3], 8), 2), rk[6], 1), _MM_SHUFFLE(1, 0, 3, 2)); // rk[13]
		...
		bool rijndael224_256_impl_aesni::init(const uint8_t* key, block_cipher::direction direction)
		{
			...
			if (direction == block_cipher::decryption)
			{
				...
				__m128i t4 = _mm_shuffle_epi32(_mm_blend_epi32(_mm_blend_epi32(_mm_srli_si128(rk[4], 4), rk[5], 1), rk[1], 8), _MM_SHUFFLE(3, 0, 2, 1)); // rk[22]
				...
				rk[21] = t1;
				t1 = _mm_shuffle_epi32(_mm_blend_epi32(_mm_blend_epi32(_mm_srli_si128(rk[11], 4), rk[12], 1), rk[8], 8), _MM_SHUFFLE(3, 0, 2, 1)); // rk[15]
