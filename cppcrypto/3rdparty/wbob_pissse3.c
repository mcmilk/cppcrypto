...
ALIGN(0x10) static const uint8_t RCON[16*4*16] = {
...
/* mini S-Boxes */
ALIGN(0x10) static const uint8_t SBOXES[48] = {
...
ALIGN(0x10) static const uint8_t CONSTANTS[80] = {
...
void wbob_pi(uint8_t *s512, uint8_t* ll)
{
    ...
    y0 = _mm_loadu_si128((__m128i*)(&s512[ 0]));
    y1 = _mm_loadu_si128((__m128i*)(&s512[16]));
    y2 = _mm_loadu_si128((__m128i*)(&s512[32]));
    y3 = _mm_loadu_si128((__m128i*)(&s512[48]));

    /* slice going in -- packs two columns per xmm */
    TRANSPOSE(x,y);

    y0 = _mm_loadu_si128((__m128i*)(&ll[0] ));
    y1 = _mm_loadu_si128((__m128i*)(&ll[16]));
    y2 = _mm_loadu_si128((__m128i*)(&ll[32]));
    y3 = _mm_loadu_si128((__m128i*)(&ll[48]));

    TRANSPOSE(s, y);

    for (r = 0; r < 10; r++) {                      // 12 rounds
        ...
        /* ShiftColumns */
        s0 = _mm_shuffle_epi8(y0, rot0);
        s1 = _mm_shuffle_epi8(y1, rot1);
        s2 = _mm_shuffle_epi8(y2, rot2);
        s3 = _mm_shuffle_epi8(y3, rot3);
        ...
        /* this basically compensates for the fact that
           we are packing two columns in each xmm but
           need complete diffusion across the rows */
        v4 = _mm_alignr_epi8(s2, s1, 8); // 3 4 L H
        v5 = _mm_alignr_epi8(s3, s2, 8); // 5 6
        v6 = _mm_alignr_epi8(s0, s3, 8); // 7 0
        v7 = _mm_alignr_epi8(s1, s0, 8); // 1 2

        v0 = _mm_xor_si128(v4, s3);
        v1 = _mm_xor_si128(v5, s0);
        v2 = _mm_xor_si128(v6, s1);
        v3 = _mm_xor_si128(v7, s2);
        ...
        y0 = _mm_xor_si128(y0, s1);
        y1 = _mm_xor_si128(y1, s2);
        y2 = _mm_xor_si128(y2, s3);
        y3 = _mm_xor_si128(y3, s0);
        ...
        y0 = _mm_xor_si128(y0, s0);
        y1 = _mm_xor_si128(y1, s1);
        y2 = _mm_xor_si128(y2, s2);
        y3 = _mm_xor_si128(y3, s3);
        ...
        /* AddRoundKey */
        s0 = _mm_xor_si128(y0, x0);
        s1 = _mm_xor_si128(y1, x1);
        s2 = _mm_xor_si128(y2, x2);
        s3 = _mm_xor_si128(y3, x3);
    }

    /* unslice going out */
    TRANSPOSE(y,s);

    _mm_storeu_si128((__m128i*)(&ll[ 0]),y0);
    _mm_storeu_si128((__m128i*)(&ll[16]),y1);
    _mm_storeu_si128((__m128i*)(&ll[32]),y2);
    _mm_storeu_si128((__m128i*)(&ll[48]),y3);
}
