...
// SSSE3 version:
//#define SWAP8(x)  (x) = _mm_shuffle_epi8((x),swap8)
//#define SWAP16(x)  (x) = _mm_shuffle_epi8((x),swap16)
...
#define round_function(nn,r)                                                              \
      ...
      lineartransform_R##nn(y0,y2,y4,y6,y1,y3,y5,y7);
		...
		void jh_impl_sse2::F8(const uint8_t *buffer)
		{
			// Constants for SSSE3 version:
			//const __m128i swap8 = _mm_set_epi8(14, 15, 12, 13, 10, 11, 8, 9, 6, 7, 4, 5, 2, 3, 0, 1);
			//const __m128i swap16 = _mm_set_epi8(13, 12, 15, 14, 9, 8, 11, 10, 5, 4, 7, 6, 1, 0, 3, 2);
			uint64 i;
			...
			y0 = XOR(y0, LOAD(buffer));
			y1 = XOR(y1, LOAD(buffer + 16));
			y2 = XOR(y2, LOAD(buffer + 32));
			y3 = XOR(y3, LOAD(buffer + 48));
			...
			y4 = XOR(y4, LOAD(buffer));
			y5 = XOR(y5, LOAD(buffer + 16));
			y6 = XOR(y6, LOAD(buffer + 32));
			y7 = XOR(y7, LOAD(buffer + 48));
